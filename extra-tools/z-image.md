...
> People are using latent upscale with z-image because the artifacts are somehow being interpreted as "hey this should be sharp and detailed" by the model.
> But it is technically a terrible way to upscale an image, you need high denoise to hide the artifacts. Upscaling a latent that way is basically
> like downscaling the image by 8, then upscaling by 1.5. So it's really pixelated + other artifacts.
...
> give zimage more direction.... it just needs it in a format it can't ignore.
> {background: details here, body: Legs: muscular, tan, with a tattoo of a anchor on his upper left thigh"
> Works really well for character, background, layout, etc.
...
One more alternative CLIP for z-image-turbo: [BennyDaBall/qwen3-4b-Z-Image-Engineer](https://huggingface.co/BennyDaBall/qwen3-4b-Z-Image-Engineer); contains
...
- [ZImage_Details_Inject.json](../workflows/z-image/dm-ZImage_Details_Inject.json) also by [Dream Making](https://ko-fi.com/dreammaking) requires [GH:RamonGuthrie/ComfyUI-RBG-SmartSeedVariance](https://github.com/RamonGuthrie/ComfyUI-RBG-SmartSeedVariance)
